...
const config = {
  ...
  telegram: {
    ...
    textLimit: 4_096,
    // Telegram รับราว 1 ข้อความ/วินาที ต่อห้องแชท เว้นจังหวะไว้กันยิงรัวจนโดน 429
    sendGapMs: 1_200,
    // ส่งไม่ผ่านติดกันกี่ครั้ง = รอเท่าไรก่อนลองใหม่ (ครั้งที่ 5 ขึ้นไปใช้ค่าสุดท้าย)
    // เดิมยิงซ้ำทุก 30 วิไม่หยุด ทำให้ Telegram เพิ่มโทษจาก retry_after 8 เป็น 574 วินาที
    backoffMs: [30_000, 60_000, 300_000, 900_000],
    // เพดานกันกรณี Telegram ส่ง retry_after มาสูงผิดปกติ แล้วเงียบยาวข้ามวัน
