...
const RepairCreate = () => {
  ...
  // โหลดข้อมูลจาก state ที่ส่งมาจากหน้าก่อนหน้า (ถ้ามี)
  useEffect(() => {
    if (location.state) {
      ...
      if (savedItems && savedItems.length > 0) {
        ...
        // สร้างแผนที่เก็บ stockQuantity เดิมของแต่ละรายการ
        const map = {};
      ...
      // เลื่อนหน้าจอถ้ามีการระบุให้เลื่อน
      ...
      // เก็บ state ที่จำเป็นไว้ใน history state เพื่อไม่ให้หายไปเมื่อรีเฟรช
      const preserved = {
  ...
  const onInvalid = (errs) => {
    ...
    try {
      ...
      // ไม่ต้องทำอะไรถ้า setFocus ล้มเหลว
    }
  ...
  const handleAddItemToRepair = (item) => {
    ...
    // เลื่อนหน้าจอไปด้านล่างสุดเพื่อแสดงรายการที่เพิ่มเข้ามาใหม่
    setTimeout(() => {
      window.scrollTo({
        top: document.body.scrollHeight,
        behavior: "smooth",
    ...
    <div className="shadow-primary flex min-h-[100svh] flex-col xl:min-h-[calc(100vh-73px)] xl:flex-row xl:items-start xl:gap-[16px] xl:bg-transparent xl:px-[16px] xl:pt-[24px] xl:pb-[24px] xl:shadow-none">
      <div className="xl:shadow-primary flex flex-1 flex-col xl:h-fit xl:w-1/2 xl:flex-initial xl:rounded-2xl xl:bg-white">
        ...
        >
            ...
            <ComboBox
              ...
              onChange={(value) => {
                ...
                setValue("model", "", {
                  shouldValidate: true,
                  shouldTouch: true,
                }); // เคลียร์รุ่นรถเมื่อเปลี่ยนยี่ห้อ
              }}
          ...
          {/* ช่องว่างด้านล่างเพื่อแยกรายการซ่อมในมุมมอง Desktop */}
